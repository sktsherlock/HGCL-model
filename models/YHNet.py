...
class YHNet(torch.nn.Module):
    def __init__(self, config, args, layers, mid):
        super(YHNet, self).__init__()
        ...
        self.conv2 = GIN( self.num_features, self.mid, self.num_layers)
        self.conv3 = GIN( self.num_features, self.mid, self.num_layers)

        self.pool1 = CONPool(self.num_features, ratio=self.pooling_ratio) 
        self.pool2 = CONPool(self.num_features, ratio=self.pooling_ratio)
        self.pool3 = CONPool(self.num_features, ratio=self.pooling_ratio)
        ...
        self.M1M2_gl_loss, self.M2M3_gl_loss, self.M3M1_gl_loss = 0.0, 0.0, 0.0
        self.dis_loss1, self.dis_loss2, self.dis_loss3 =0.0, 0.0, 0.0 
    ...
    def forward(self, x, edge_index, batch, device):
        ...
        X_M2, M2_P = self.conv2(x1, edge_index1, batch1)

        M2_con = self.projection_head_2(M2_P)
        x2, edge_index2,_, batch2, loss2,perm_positive, perm_negative = self.pool2(x1, edge_index1, device, None, batch1)
        x_l2 = x1[perm_positive] 
        batch_l2 = batch1[perm_positive]

        l2_A, _  = f_adj(edge_index1,edge_attr = None, perm = perm_positive, num_nodes = int(x1.shape[0]))
        x_l2, L2_readout = self.conv2(x_l2, l2_A, batch_l2)
        L2_con = self.projection_head_2(L2_readout)

        x_s2 = x1[perm_negative] 
        batch_s2 = batch1[perm_negative]
        s2_A, _  = f_adj(edge_index1,edge_attr = None,perm = perm_negative,num_nodes = int(x1.shape[0]))
        x_s2, S2_readout = self.conv2(x_s2, s2_A, batch_s2)
        S2_con = self.projection_head_1(S2_readout)

        #self.cl_pool2_loss = PAP1Loss.compute(self, anchor = M2_con, sample1 = L2_con, sample2 = S2_con )
        self.cl_pool2_loss = InfoNCE.compute(self, L2_con, S2_con) #+ InfoNCE.compute(self, M2_con, S2_con)
        #self.cl_pool2_loss = (global_global_loss_(M2_con, S2_con) + global_global_loss_(M2_con, L2_con) ) /2
        self.dis_loss2 = loss2

        X_M3, M3_P = self.conv3(x2, edge_index2, batch2)

        M3_con = self.projection_head_3(M3_P)
        x3, edge_index3,_, batch3, loss3,perm_positive, perm_negative = self.pool3(x2, edge_index2, device, None, batch2)

        x_l3 = x[perm_positive] 
        batch_l3 = batch2[perm_positive]

        l3_A, _  = f_adj(edge_index2,edge_attr = None, perm = perm_positive, num_nodes = int(x2.shape[0]))
        x_l3, L3_readout = self.conv3(x_l3, l3_A, batch_l3)
        L3_con = self.projection_head_3(L3_readout)

        x_s3 = x2[perm_negative] 
        batch_s3 = batch2[perm_negative]
        s3_A, _  = f_adj(edge_index2,edge_attr = None,perm = perm_negative,num_nodes = int(x2.shape[0]))
        x_s3, S3_readout = self.conv3(x_s3, s3_A, batch_s3)
        S3_con = self.projection_head_3(S3_readout)

        #self.cl_pool3_loss = PAP1Loss.compute(self, anchor = M3_con, sample1 = L3_con, sample2 = S3_con )
        #self.cl_pool3_loss = (global_global_loss_(M3_con, S3_con) + global_global_loss_(M3_con, L3_con) ) /2
        self.cl_pool3_loss = InfoNCE.compute(self,L3_con, S3_con)# + InfoNCE.compute(self, M3_con, S3_con) #   +InfoNCE.compute(self,M3_con,S3_con)) /2
        
        self.dis_loss3 = loss3 


        # X_M3, M3_P = self.conv3(x2, edge_index2)
        # x3, edge_index3, batch3,  loss3 = self.pool3(x2, edge_index2, None, batch2)
        # self.dis_loss3 = loss3

        self.infonce_M1M2 = InfoNCE.compute(self, M1_con, M2_con)
        self.infonce_M2M3 = InfoNCE.compute(self, M2_con, M3_con)
        self.infonce_M3M1 = InfoNCE.compute(self, M3_con, M1_con)

        x = (M1_P + M2_P + M3_P)/3
        #x = torch.cat((M1_P, M2_P, M3_P), 1)
        return x, M1_P
    
    def compute_Farloss(self):
        return (self.dis_loss1 + self.dis_loss2 + self.dis_loss3)/3#(self.dis_loss1 + self.dis_loss2 +self.dis_loss3) / 3
    

    def compute_inner(self):
        return   (self.cl_pool1_loss + self.cl_pool2_loss + self.cl_pool3_loss) /3
    def compute_hier(self):
        return (self.infonce_M1M2 + self.infonce_M2M3 + self.infonce_M3M1) / 3#+self.infonce_M2M3 +self.infonce_M3M1) 
