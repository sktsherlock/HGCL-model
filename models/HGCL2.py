...
class CONPool(torch.nn.Module):
    ...
    def forward(self, x, edge_index, device, edge_attr=None, batch=None):
        ...
        s_lp = self.l_pooling(x_tl,
                              edge_index).squeeze()  # np.squeeze（）函数可以删除数组形状中的单维度条目，即把shape中为1的维度去掉，但是对非单维的维度不起作用。
...
class HGCL2(torch.nn.Module):
    def __init__(self, config, args, layers, mid):
        super(HGCL2, self).__init__()
        ...
        self.M1M2_gl_loss, self.M2M3_gl_loss = 0.0, 0.0
        self.dis_loss1, self.dis_loss2 = 0.0, 0.0
    ...
    def forward(self, x, edge_index, batch, device):
        ...
        x1, edge_index1, _, batch1, loss1, perm_positive, perm_negative = self.pool1(x, edge_index, device=device,
                                                                                     edge_attr=None,
                                                                                     batch=batch)  # edge_index表示A 邻接矩阵
        ...
        x2, edge_index2, _, batch2, loss2, perm_positive, perm_negative = self.pool2(x1, edge_index1, device, None,
                                                                                     batch1)
    ...
    def compute_Farloss(self):
        return (self.dis_loss1 + self.dis_loss2) / 2  # (self.dis_loss1 + self.dis_loss2 +self.dis_loss3) / 3

    def compute_inner(self):
        return (self.cl_pool1_loss + self.cl_pool2_loss) / 2
