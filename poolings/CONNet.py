...
import numpy as np
from utils import global_global_loss_, global_global_negative_loss_,f_adj, local_global_loss_, PAPloss, PAP1Loss, Far
from MLP import MLP
...
def fuse_x(perm1, perm2, batch1,x1, x2, o_batch, edge_index, edge_attr, x):
    device = x1.device
    dtype = x1.dtype
    ...
    edge_index, edge_attr = f_adj(edge_index, edge_attr, perm, num_nodes= int(x.shape[0]))
    edge_index.to(device)
    return x3, new_batch, edge_index, edge_attr

class CONNet(torch.nn.Module):
    def __init__(self, config, args, batch_size, layers, mid,  in_channels=128, ratio=0.9, non_lin=torch.tanh):
        ...
        self.SP1 = TopKPooling(in_channels = self.num_features, ratio = self.ratio ) # 3
        # self.LP11 = GraphConv(self.inputdim, 1) 
        # self.SP11 = GraphConv(self.inputdim, 1)
        # self.LP1_M1 = GIN( self.num_features, self.mid, self.num_layers)
        # self.SP1_M1 = GIN( self.num_features, self.mid, self.num_layers)
        ...
        # self.LP22 = GraphConv(self.inputdim, 1) 
        # self.SP22 = GraphConv(self.inputdim, 1)
        # self.LP2_M2 = GIN( self.num_features, self.mid, self.num_layers)
        # self.SP2_M2 = GIN( self.num_features, self.mid, self.num_layers)
        ...
        # self.LP33 = GraphConv(self.inputdim, 1) 
        # self.SP33= GraphConv(self.inputdim, 1)
        # self.LP3_M3 = GIN( self.num_features, self.mid, self.num_layers)
        # self.SP3_M3 = GIN( self.num_features, self.mid, self.num_layers)
        # 损失定义
        self.L1M1_loss = 0.
        self.S1M1_loss = 0.
        self.L1S1_loss = 0.
        ...
        self.projection_head_2 = MLP(in_channels= self.inputdim , hidden_channels= self.hidden, out_channels = self.out)
        self.projection_head_3 = MLP(in_channels= self.inputdim , hidden_channels= self.hidden, out_channels = self.out)
    ...
    def forward(self, x, edge_index, batch, device, edge_attr=None):
        ...
        # localpooling 1 
        X_L1, edge_index_local1, edge_attr_local1, batch_l1, perm1_1, _ = self.LP1(x, edge_index, edge_attr = edge_attr, batch = batch, attn = None)
        X_L11, L1_readout = self.M1(X_L1, edge_index_local1,batch_l1)
        L1_Re= gap(X_L1, batch_l1)
        ...
        X_S11, S1_readout = self.M1(X_S1, edge_index_sem1, batch_s1)
        S1_Re = gap(X_S1, batch_s1) 
        ...
        #self.YH_loss1 = PAPloss.compute(self, anchor = M1_con, sample1 = L1_con, sample2 = S1_con )
        self.YH_loss1 = PAP1Loss.compute(self, anchor = M1_con, sample1 = L1_con, sample2 = S1_con )
        #self.YH_loss1 = ( global_global_loss_(M1_con,L1_con) + global_global_loss_(M1_con, S1_con) )/2
        self.YH_far1  = Far.compute(self, L1_con, S1_con) #之前是L1 Re
        ...
        x1, batch1, edge_index1, edge_attr1 = fuse_x(perm1_1, perm2_1, batch_l1, X_L1, X_S1, batch, edge_index, edge_attr, x)


        #d=第二层
        X_M2, M2_P = self.M2(x1, edge_index1, batch1)
        M2_readout = gap(X_M2,batch1) #
        M2_con = self.projection_head_2(M2_P)
        # localpooling 1 
        X_L2, edge_index_local, edge_attr_local, batch_l2, perm1_2, _ = self.LP2(x1, edge_index1, edge_attr = edge_attr1, batch = batch1, attn = None)
        _, L2_readout = self.M2(X_L2, edge_index_local,batch_l2)
        L2_Re = gap(X_L2, batch_l2)
        L2_con = self.projection_head_2(L2_readout)
        X_S2, edge_index_local2, edge_attr_local2, batch_s2, perm2_2, _ = self.SP2(x1, edge_index1, edge_attr = edge_attr1, batch = batch1, attn = None)
        _, S2_readout = self.M2(X_S2, edge_index_local2 ,batch_s2)
        #X_S2, edge_index_local2, edge_attr_local2, batch_s2, perm2_2, _ = self.SP2(x, edge_index, edge_attr = None, batch = batch, attn = None)
        S2_Re = gap(X_S2, batch_s2) 
        S2_con = self.projection_head_2(S2_readout)
        ...
        #self.YH_loss2 = PAPloss.compute(self, anchor = M2_con, sample1 = L2_con, sample2 = S2_con )
        self.YH_loss2 = PAP1Loss.compute(self, anchor = M2_con, sample1 = L2_con, sample2 = S2_con )
        #self.YH_loss2 = ( global_global_loss_(M2_con,L2_con) + global_global_loss_(M2_con, S2_con) )/2
        self.YH_far2  = Far.compute(self, L2_con, S2_con)
        #z2 = M2_readout + L2_readout + S2_readout
        z2 = M2_P#z2 = torch.cat((M2_P, (L2_readout + S2_readout)/2), 1)

        x2, batch2, edge_index2, edge_attr2 = fuse_x(perm1_2, perm2_2, batch_l2, X_L2, X_S2, batch1, edge_index1, edge_attr1, x1)
        ...
        M3_con = self.projection_head_3(M3_P)
        # localpooling 1 
        X_L3, edge_index_local, edge_attr_local, batch_l3, perm1_3, _ = self.LP3(x2, edge_index2, edge_attr = edge_attr2, batch = batch2, attn = None)
        _, L3_readout = self.M3(X_L3, edge_index_local,batch_l3)
        L3_Re = gap(X_L3, batch_l3)
        L3_con = self.projection_head_3(L3_readout)
        X_S3, edge_index_local3, edge_attr_local3, batch_s3, perm2_3, _ = self.SP3(x2, edge_index2, edge_attr = edge_attr2, batch = batch2, attn = None)
        _, S3_readout = self.M3(X_S3, edge_index_local3,batch_l3)
        S3_Re = gap(X_S3, batch_s3) 
        S3_con = self.projection_head_3(S3_readout)
        ...
        #self.YH_loss3 = PAPloss.compute(self, anchor = M3_con, sample1 = L3_con, sample2 = S3_con )
        self.YH_loss3 = PAP1Loss.compute(self, anchor = M3_con, sample1 = L3_con, sample2 = S3_con )
        #self.YH_loss3 = ( global_global_loss_(M3_con,L3_con) + global_global_loss_(M3_con, S3_con) )/2
        self.YH_far3  = Far.compute(self, L3_con, S3_con)
    ...
    def compute_YH_loss(self):
        return (self.YH_far1 + self.YH_far2 + self.YH_far3)/3#(self.YH_loss1+self.YH_loss2+self.YH_loss3)/3 +(self.YH_far1 + self.YH_far2 + self.YH_far3)/3
    
    def compute_H(self):
        return   (self.YH_loss1+self.YH_loss2+self.YH_loss3)/3 #self.H_loss + (self.YH_loss1+self.YH_loss2+self.YH_loss3)/3
